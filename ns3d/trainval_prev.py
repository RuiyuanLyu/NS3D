...
def train_epoch(epoch, trainer, train_dataloader, meters, all_scans_in_dict):
    ...
    with tqdm_pbar(total=nr_iters) as pbar:
        for i in range(nr_iters):
            ...
            # dict_keys(['class_labels', 'context_size', 'objects', 'target_class', 'target_pos', 'target_class_mask', 'tokens', 'is_nr3d', 'scan_id', 'utterance', 'stimulus_id'])
            class_labels = feed_dict['class_labels'] # torch.int64([16, 10])
            context_size = feed_dict['context_size'] # torch.int64([16]), each value is 10
            objects = feed_dict['objects'] # torch.float32([16, 10, 1024, 6])
            target_class = feed_dict['target_class'] # torch.int64([16])
            target_pos = feed_dict['target_pos'] # torch.int64([16]) 
            target_class_mask = feed_dict['target_class_mask'] # torch.bool([16, 10])
            tokens = feed_dict['tokens'] # torch.int64([16, 26]) 
            is_nr3d = feed_dict['is_nr3d'] # torch.bool([16])
            scan_id = feed_dict['scan_id'] # list of str, len=16
            utterance = feed_dict['utterance'] # list of (long) str, len=16
            stimulus_id = feed_dict['stimulus_id'] # list of str, len=16, example: scene0392_00-door-2-29-25
            # stimulus_id format: sceneid-instanceType-numObjects-targetid-distractorids
